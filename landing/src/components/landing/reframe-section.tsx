...
function ReframeCard({ type, label, children }: { type: "endurance" | "strength"; label: string; children: ReactNode }) {
...
export function ReframeSection() {
  ...
  return (
    <SectionWrapper bg="light" className="py-22 px-10 text-center max-tablet:py-16 max-tablet:px-6 max-sm:py-14 max-sm:px-5" id="reframe">
      <SectionInner maxWidth={800}>
        <Reveal>
          <Heading size="lg" color="dark" align="center" className="max-sm:text-[1.6rem] max-[380px]:text-[1.35rem]">
            {t("reframe.heading")}
          </Heading>
        </Reveal>
        <Reveal delay={0.08}>
        ...
        <Reveal delay={0.16}>
          <div className="flex justify-center gap-11 mb-12 py-7 border-t border-b border-brand-mid max-sm:gap-5 max-sm:flex-wrap max-[380px]:gap-3">
            {ELEMENT_SYMBOLS.map((symbol) => (
              <ElementBadge key={symbol} symbol={symbol} name={t(`elements.${symbol}`)} />
            ))}
          </div>
        </Reveal>
        <div className="grid grid-cols-2 gap-4 text-left max-sm:grid-cols-1 max-sm:gap-3">
          <Reveal delay={0.08}>
            <ReframeCard type="endurance" label={t("reframe.enduranceLabel")}>
              <Trans i18nKey="reframe.enduranceBody" components={{ strong: <strong /> }} />
            </ReframeCard>
          </Reveal>
          <Reveal delay={0.16}>
            <ReframeCard type="strength" label={t("reframe.strengthLabel")}>
              <Trans i18nKey="reframe.strengthBody" components={{ strong: <strong /> }} />
            </ReframeCard>
          </Reveal>
        </div>
        <Reveal delay={0.24}>
